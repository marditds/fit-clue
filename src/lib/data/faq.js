export const faq = [
    {
        category: 'General',
        questions: [{
            title: 'Is FitClue free?',
            desc: 'Yes, FitClue is free to use. There are no plans to charge users anytime soon, and if this ever changes, we will inform you promptly.',
        },
        {
            title: 'Is there an age requirement for this platform?',
            desc: 'Since the content on this platform is user-generated, we require all users to be of legal age to avoid exposure to inappropriate or potentially harmful material. If we discover that a user is underage, their account will be terminated without notice.',
        }]
    },
    {
        category: 'Accounts & sign-in',
        questions: [{
            title: 'Do I need an Instagram account to use FitClue?',
            desc: 'No. You do not need an Instagram account to use FitClue. Instagram content may appear within FitClue, but it can be viewed directly without signing in to Instagram.'
        },
        {
            title: 'Do I need a FitClue account?',
            desc: 'No account is required to browse most FitClue content. However, some content and certain actions (such as saving, interacting, or accessing restricted features) require a FitClue account.'
        },
        {
            title: 'Can I sign in using Instagram or other social media accounts?',
            desc: 'No, you cannot. To register with FitClue, you need a valid email address, which will be used to sign up, sign in, and recover your password if needed.'
        }]
    },
    {
        category: 'Platform & social media',
        questions: [{
            title: 'Are you associated with Instagram?',
            desc: 'FitClue is not associated with, affiliated with, sponsored by, promoted by, or endorsed by Instagram or any other social media platform.',
        },
        {
            title: 'Can I interact with other users?',
            desc: 'You can interact with other users by commenting on their posts. Direct messaging is not available.',
        }]
    }
]